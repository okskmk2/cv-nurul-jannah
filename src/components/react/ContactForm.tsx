...
export function ContactForm({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    const hint = new URLSearchParams(window.location.search).get("product");
    if (!hint) return;
    setForm((prev) => ({
      ...prev,
      mainProducts: prev.mainProducts || hint,
      message: prev.message || `Interested in: ${hint}`,
    }));
  }, []);
      ...
      <div className="space-y-4">
        ...
        <label className="block">
          ...
          <input
            ...
            placeholder={t(locale, "contact.addressPh")}
            className={fieldClass}
          />
        </label>
        ...
        <label className="block">
          <span className="mb-1.5 block text-sm font-medium text-brand-forest">
            {t(locale, "contact.tradePerson")} *
          </span>
          <input
            required
            value={form.tradePerson}
            onChange={(e) => update("tradePerson", e.target.value)}
            placeholder={t(locale, "contact.tradePersonPh")}
            className={fieldClass}
          />
        </label>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block">
            ...
            <input
              ...
              placeholder={t(locale, "contact.industryPh")}
              className={fieldClass}
            />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-brand-forest">
              {t(locale, "contact.mainProducts")} *
            </span>
            <input
              required
              value={form.mainProducts}
              onChange={(e) => update("mainProducts", e.target.value)}
              placeholder={t(locale, "contact.mainProductsPh")}
              className={fieldClass}
            />
          </label>
        </div>
